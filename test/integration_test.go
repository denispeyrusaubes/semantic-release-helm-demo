...
func TestPodDeploysContainerImage(t *testing.T) {
	...
	options := &helm.Options{
		SetValues: map[string]string{"image.tag": "nginx:1.15.8"},
	}
	...
	label := fmt.Sprintf("app.kubernetes.io/name=example,app.kubernetes.io/instance=%s", releaseName)

	k8s.WaitUntilNumPodsCreated(t, kubectlOptions, v1.ListOptions{LabelSelector: label}, 1, 5, 5*time.Second)
	pods := k8s.ListPods(t, kubectlOptions, v1.ListOptions{LabelSelector: label})

	verifyNginxPod(t, kubectlOptions, pods[0].Name)
}
